...
void Database::initDB()
{
    ...
    mDb.exec("CREATE TABLE IF NOT EXISTS files (id INTEGER PRIMARY KEY AUTOINCREMENT, name varchar(10) NOT NULL UNIQUE); ");
    mDb.exec("CREATE TABLE IF NOT EXISTS permissions (file_id INTEGER, permission INTEGER, FOREIGN KEY (file_id) REFERENCES files (id) ON DELETE CASCADE, PRIMARY KEY (file_id, permission));");
    transaction.commit();
}

void Database::setPermission(const std::string &path, const int32_t &permission)
{
    SQLite::Statement setPermission(mDb, "INSERT INTO permissions VALUES (?, ?);");
...
bool Database::hasPermission(const std::string &path, const int32_t &permission)
{
    SQLite::Statement getPermissionQuery(mDb, "SELECT * FROM permissions WHERE file_id = ? AND permission = ?;");
