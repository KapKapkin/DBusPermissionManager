...
PermissionsAdaptor::PermissionsAdaptor(sdbus::IConnection &connection,
                                       sdbus::ObjectPath path)
    : AdaptorInterfaces(connection, std::move(path)) {
  registerAdaptor();
}

PermissionsAdaptor::~PermissionsAdaptor() { unregisterAdaptor(); }
...
void PermissionsAdaptor::RequestPermission(const int32_t &permissionEnumCode) {
  ...
  std::string path_to_exe = [&]() {
    try {
      ...
      throw sdbus::Error(
          sdbus::Error::Name("com.system.permissions.FileError"),
          "can't find sender path or something else went wrong.");
    }
  }();
  db::Database::Instance().setPermission(path_to_exe, permissionEnumCode);
}
bool PermissionsAdaptor::CheckApplicationHasPermission(
    const std::string &applicationExecPath, const int32_t &permissionEnumCode) {
  std::string path_canonical = [&]() {
    try {
      ...
      throw sdbus::Error(
          sdbus::Error::Name("com.system.permissions.FileError"),
          "can't find file with given path or something else went wrong.");
    }
  }();

  return db::Database::Instance().hasPermission(path_canonical,
                                                permissionEnumCode);
}
